...
8. また、コード全体ではなく、特定の部分に対して問題点を聞いてみるとどのような提案が出るかを確認するために、`main` メソッド全体をマウスで選択してから、右クリックして「Copilot」>「Add Selection to Chat」を選択してみてください。
...
2. ファイルの中の43行目に `<!-- TODO: Buttons -->` というコメントまでスクロールしてください。
...
6. ファイルの中の13行目にある `// TODO: Add operator` というコメントの下にある処理の中に、べき乗の計算を行う処理を追加するために、18行目にある `'devide'` の処理が書かれている行の下に新しい行を追加してください。
...
![unittest](./assets/ex1_06.png)

これは1つのケースしか網羅していないので、いくつかのパターンを追加してみてください。
</details>
...
> モデルの最大応答を調整できる場合は、最大応答を大きめにセットするとより多くの回答が得られます。AI からの回答が途中で途切れてしまった場合はこの設定をまずは確認してみてください。

</div>

3. 結果が返ってきましたら、その内容を吟味してください。

<div class="task" data-title="追加課題">

> このスケルトンコードを元に、実際に Java のクラスとメソッドを実装してみてください。このとき、GitHub Copilot も活用してみてください。

</div>

### 画面のスクリーンショットからコードの初期アイデアをもらう

1. 以下の画像をコピーもしくは保存してください。

![uml](./assets/d2clayout.png)

2. 先ほどの画像を以下のプロンプトと一緒に AI チャットサービスに送信してください。（以下の画像は Azure AI Studio で利用している例です。）

`画面イメージからReact.jsのコードを生成してください。これは経費精算システムの画面で、日付、摘要、科目、金額を入力するテーブルがあり、テーブルはスクロール可能なテーブルになっています。また、保存、申請はそれぞれ機能ボタンとして動作し、バックエンドにあるマイクロサービスにテーブルの内容をPOSTで送信します。`

![aistudio](./assets/ex2_02.png)
...
3. 結果が返ってきましたら、その内容を吟味してください。
4. 画面のレイアウトを元に、React のコードを実装してみてください。

<details>
<summary>React アプリの作成例</summary>

- Exercise3 ディレクトリをターミナル上で開いてください。
- `npx create-react-app my-app` をコマンドで実行してください。`my-app` はアプリが配置されるディレクトリです。
- 新規作成が完了しましたら `cd my-app` を実行し、`npm start` を実行してください。テンプレートの React アプリがローカル上で起動するはずです。

</details>


5. 実装が完了したら、React アプリケーションを起動して、画面のレイアウトが正常に表示されることを確認してください。

<details>
<summary>実装例</summary>

![reactcode](./assets/ex2_03.png)

</details>

6. もし、意図していない挙動やレイアウトだった場合は、GitHub Copilot や AI チャットをうまく活用しながらコードの実装を行ってください。

<div class="task" data-title="追加課題">

> AI に投げるプロンプトを変更し、回答がどのように変化するかを確認してみてください。また、どのようなプロンプトを準備すると、このユースケースにおいてより想定通りの回答が得られるかを考えてみてください。
...
1. 以下のテキストを AI チャットに投げてみてください。

`以下の処理の内容をMarmeid記法でシーケンス図にしてください。 ライフラインの定義は、以下の通りです。 
・システムに入力を行うユーザ
・ユーザの入力を受け取るインターフェース
・インターフェースからのリクエストを処理するビジネスロジック
・ビジネスロジックからの命令を受け、データベースに書き込むデータベースオブジェクト
システムはユーザが経費の申請を行うためのシステムのシーケンスの一部です。
処理は以下の内容です。
ユーザはインターフェースに経費を入力する。
ユーザはインターフェースに経費の「保存」を指示する
インターフェースはビジネスロジックに経費の「保存」を要求する
ビジネスロジックは全ての経費をチェックし、入力エラーがある場合にはインターフェースにエラーを返す。
ビジネスロジックからエラーが返ってきた場合は、インターフェースはユーザーにエラーを通知する。
ビジネスロジックで経費のチェックが正常に終了した場合、ビジネスロジックはデータベースオブジェクトに経費の「保存」を依頼する。
経費の保存を依頼されたデータベースオブジェクトは、経費の保存を行い、結果をビジネスロジックに返す。
データベースオブジェクトから保存結果を受け取ったビジネスロジックは、成功もしくは失敗をインターフェースに返す
インターフェースはユーザーに保存結果を返す。
`

2. Mermaid 記法にのっとったテキストが返答されましたら、Mermaid の [Live Editor](https://mermaid.live/) へアクセスし、UML 図を作成してください。サンプルが最初から入っている場合は、すべて削除してから AI から得たテキストを入力してください。

![marmaidlive](./assets/ex2_05.png)

<details>
<summary>例</summary>

![marmaidlive](./assets/ex2_04.png)
